...
import { loadGraph, reachableByType } from "./graph";
import type { RiskBand, RiskProfile } from "./types";
...
async function gatherInputs(): Promise<SupplierRiskInputs[]> {
  const db = await getDb();
  const graph = await loadGraph();

  const suppliers = await db.query<{ id: string; label: string; props: Record<string, unknown> }>(
    "SELECT id, label, props FROM nodes WHERE type = 'Supplier'"
  );

  const out: SupplierRiskInputs[] = [];
...
function scoreFor(i: SupplierRiskInputs): number {
...
export async function recomputeAllRiskScores(): Promise<RiskProfile[]> {
  const db = await getDb();
  ...
  for (const i of inputs) {
    ...
    const prev = await db.query<{ score: number }>(
    ...
    const cur = await db.query<{ props: Record<string, unknown> }>(
    ...
    await db.query(
      "INSERT INTO risk_history (supplier_id, score, delta) VALUES ($1,$2,$3)",
      [i.id, score, delta]
    );
...
function buildSummary(i: SupplierRiskInputs, score: number, delta: number): string {
...
export async function getRiskProfiles(): Promise<RiskProfile[]> {
  const db = await getDb();
  const res = await db.query<{ id: string; label: string; props: Record<string, unknown> }>(
    "SELECT id, label, props FROM nodes WHERE type = 'Supplier'"
  );
  const profiles: RiskProfile[] = res.rows.map((s) => {
    const p = s.props as Record<string, unknown>;
    const score = Number(p.risk_score ?? p.base_risk ?? 0);
    const openCves = (p.open_cves as string[]) ?? [];
    return {
      supplier_id: s.id,
      name: s.label,
