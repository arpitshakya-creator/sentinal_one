...
import { fetchCvesForKeyword, delay } from "./nvd";
import { fetchKevForKeywords } from "./cisa";
import { recomputeAllRiskScores } from "./risk";
import type { CVE, GraphNode } from "./types";

interface SupplierRow {
  id: string;
  label: string;
  props: { keywords?: string[] } & Record<string, unknown>;
}

async function getSuppliers(): Promise<SupplierRow[]> {
  const db = await getDb();
  const res = await db.query<SupplierRow>(
    "SELECT id, label, props FROM nodes WHERE type = 'Supplier'"
  );
  return res.rows;
}

async function upsertCve(cve: CVE) {
  const db = await getDb();
  await db.query(
...
async function linkCve(nodeId: string, cveId: string, keyword: string) {
  const db = await getDb();
  await db.query(
...
async function emitThreatEvent(supplierId: string, cve: CVE) {
  const db = await getDb();
 ...
 */
export async function runIngest(opts: { throttleMs?: number } = {}): Promise<IngestResult> {
  ...
  const keywordToSuppliers = new Map<string, string[]>();
  ...
  try {
    ...
    result.errors.push(`CISA KEV: ${(e as Error).message}`);
  }
  ...
  for (let i = 0; i < keywords.length; i++) {
    ...
    try {
      ...
      result.errors.push(`NVD "${kw}": ${(e as Error).message}`);
    }
