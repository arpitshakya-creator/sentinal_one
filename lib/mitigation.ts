import { loadGraph, type LoadedGraph } from "./graph";
import type {
  AlternateSupplier,
  ImpactAssessment,
  MitigationAction,
...
const REGION_ADJACENCY: Record<string, Record<string, number>> = {
...
function proximityScore(supplierRegion: string, plantRegions: string[]): number {
  if (plantRegions.length === 0) return 0.5;
  const scores = plantRegions.map(
    (pr) => REGION_ADJACENCY[supplierRegion]?.[pr] ?? 0.3
  );
  return Math.max(...scores);
}

function componentsOf(graph: LoadedGraph, supplierId: string): Set<string> {
...
function plantRegionsFromImpact(graph: LoadedGraph, impact: ImpactAssessment): string[] {
...
function findAlternateSuppliers(
  graph: LoadedGraph,
  ...
  const targetCategories = new Set(
    (compromised.props.component_categories as string[]) ?? []
  );
  ...
  const candidates: AlternateSupplier[] = [];
  for (const node of graph.nodes.values()) {
    ...
    const theirCats = new Set((node.props.component_categories as string[]) ?? []);
    ...
    const similarity =
      componentMatch * 0.4 + geo * 0.25 + quality * 0.2 + capacity * 0.15;

    const leadTime = Number(node.props.lead_time_days ?? 14);
    const onboarding: AlternateSupplier["onboarding_effort"] =
      similarity > 0.8 ? "low" : similarity > 0.6 ? "medium" : "high";
...
export async function buildMitigationPlan(
  supplierId: string,
  impact: ImpactAssessment
