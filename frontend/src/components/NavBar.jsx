...
export function NavBar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-line bg-bg-soft/90 backdrop-blur">
      <div className="pwc-gradient h-1 w-full" />
      <div className="mx-auto flex max-w-[1500px] items-center gap-6 px-5 py-3">
        <Link to="/" className="flex items-center gap-2">
          <span className="grid h-9 w-9 place-items-center rounded-lg pwc-gradient text-white">
          ...
          <div className="leading-tight">
            <div className="text-sm font-semibold text-slate-900">
              SupplyChain Sentinel <span className="pwc-text-gradient font-bold">AI</span>
            </div>
            ...
            return (
              <Link
                ...
                className={`rounded-lg px-3 py-1.5 text-sm transition-colors ${
                  active ? "bg-bg-elevated text-slate-900" : "text-slate-600 hover:text-slate-900"
                }`}
              >
                {l.label}
              </Link>
          ...
          {status && (
            <>
              ...
              <span
                ...
                title={
                  status.ai_enabled
                    ? "AI agents active (Gen AI Shared Services)"
                    : "Set AI_API_KEY to enable AI narrative summaries"
                }
              >
