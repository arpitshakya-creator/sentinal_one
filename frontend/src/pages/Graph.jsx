...
export default function Graph() {
  ...
  const simulateBreach = async (supplierId) => {
    setBusy(true);
    setImpact(null);
    try {
      ...
      if (data.impact) {
        const set = new Set([supplierId, ...data.impact.blast_radius_nodes]);
        setBlast(set);
...
function InspectCard({ node, onSimulate, busy }) {
  const p = node.props;
  return (
      ...
      <div className="mt-3 space-y-1 text-sm">
        {node.type === "Supplier" && (
          <>
            ...
            <Row k="Open CVEs" v={String(p.open_cves?.length ?? 0)} />
            <Row k="Vendor tech" v={p.keywords?.join(", ") ?? "—"} />
          </>
        )}
