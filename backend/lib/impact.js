...
import { loadGraph, bfsBlastRadius } from "./graph.js";
...
function componentsProvidedBy(graph, supplierId) {
  const out = [];
...
function alternateLeadTime(graph, componentId, excludeSupplierId, fallbackLead) {
  let best = null;
...
export async function assessImpact(supplierId, scenario = "ransomware") {
  ...
  const linkedCvesRes = await db.query(
