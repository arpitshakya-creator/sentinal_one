...
export async function loadGraph() {
  ...
  const nodeRes = await db.query("SELECT id, type, label, props FROM nodes");
  const edgeRes = await db.query("SELECT id, source, target, type, props FROM edges");
 ...
 */
export function bfsBlastRadius(graph, startId, sourceRisk) {
  const best = new Map();
  const queue = [{ id: startId, risk: sourceRisk, hop: 0 }];
...
export function reachableByType(graph, startId, type) {
...
export async function countReachablePlants(supplierId) {
